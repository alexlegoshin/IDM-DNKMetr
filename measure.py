"""Пакетный CLI-режим: цикл измерения по списку датчиков из config.yaml с ручной
коммутацией (пауза на Enter), пишет CSV. Основной интерактивный сценарий — UI
(см. ui.py/bench.py) — этот модуль они переиспользуют напрямую, не дублируют.
...
не меняется (см. README.md, "Потенциал к расширению").
"""

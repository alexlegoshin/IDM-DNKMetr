...
режим CC (Constant Current). Если есть — используй HardwareCCController (проще
и надёжнее). Если нет — SoftwarePIController (см. README.md).
"""
...
class SoftwarePIController:
    ...
    провоцирует колебания (см. README.md). В velocity-форме такой
    ...
    def __init__(self, supply, dmm, setpoint=0.010,
        ...
        # Значения по умолчанию изначально подобраны симуляцией на FakeMultimeter
        # (нагрузка ~800 Ом, см. tune_current_loop.py) — сходится за ~2.5 с без
        # перерегулирования на ЭТОЙ модели. На реальном ДНК-М работают приемлемо
        # без ретюнинга (см. README.md), но не оптимизированы под него специально.
        self.supply = supply
        self.dmm = dmm
        self.setpoint = setpoint
        self.kp = kp
